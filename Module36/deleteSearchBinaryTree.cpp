...
// Search Path Binary Search Tree 
...
/* 
Case 1 : node if leaf node hoi tile jast delete node
Case 2 : if you have a single child then delete the child and replace the parent for 
his child 
Case 3 : if more then one and two then  first find successer or inorder value
then replace the parent to successer 
 */
...
treeNode* delectionBST( treeNode* root, int value ){
    if( value < root->data ){
        root->leftChild = delectionBST( root->leftChild, value );
    }else if( value > root->data ){
        root->rightChild = delectionBST( root->rightChild, value );
    }   
    else {
        //Implementation of 3 case 
        if( root->leftChild == NULL ){ // Case 1 handle properly  \\ case 2 also handle
            treeNode* temp = root->rightChild;
            free(root);
            return temp;
        }
        else if( root->rightChild == NULL ){  // Case 2 handle 
            ...
            root->data = temp->data;
            root->rightChild = delectionBST( root->rightChild, temp->data);
        }
        return root;
    }
}
int main(){
    ...
    //     cout << "Value Does not Exits in the BST ";
    // }else{
    //     cout << endl;
    //     cout << "Value Exits in the BST " << endl;
    // }
    root = delectionBST(root, key);
    string afterDeletion = "";
    inOrderTraversal( root, afterDeletion );
    cout << endl;
